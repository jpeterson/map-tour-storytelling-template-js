define(["esri/map",
	...
	function init(mainView, builder) {
		...
		esri.id && esri.id.setProtocolErrorHandler(function() {
			return true;
		});
	...
	function webMapInitCallback(response) {
		...
		/**
		 * Customization to add an invisible TimeSlider to the application
		 * See ArcGIS API for JavaScript documentation for methods to interact with the TimeSlider
		 *
		 */
		...
		val = APPCFG.TIME_VALUES[0];

		if (APPCFG.TIME_TYPE == 1) {
		...
		timeExtent.startTime = new Date(APPCFG.TIME_SPAN.start);
		timeExtent.endTime = new Date(APPCFG.TIME_SPAN.end);
		timeSlider.setThumbCount(APPCFG.TIME_TYPE);
		console.log(timeSlider.thumbIndexes);
		timeSlider.createTimeStopsByTimeInterval(timeExtent, APPCFG.TIME_INTERVAL.length, APPCFG.TIME_INTERVAL.units);
		timeSlider.startup();
		...
		app.header.init(
			...
			appColors[0],
			logoURL,
			logoTarget, !app.isInBuilderMode && (
		(!isProd() && Helper.getAppID(isProd())) || isProd() && app.data.userIsAppOwnerOrAdmin()),
	...
	function updateUI(params) {
		...
		app.header.setLogoInfo(
			...
			APPCFG.HEADER_LOGO_TARGET : WebApplicationData.getLogoTarget());
