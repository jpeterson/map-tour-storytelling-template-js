define(["storymaps/maptour/core/WebApplicationData",
	...
	return function MainView() {
		...
		this.init = function(core) {
			...
			configOptions.firstRecordAsIntro = urlParams.firstRecordAsIntro ? (urlParams.firstRecordAsIntro == "true" ? true : false) : configOptions.firstRecordAsIntro;
		...
		this.webmapLoaded = function() {
			...
			if (configOptions.sourceLayerTitle) {
				var webmapLayers = app.data.getWebMapItem() && app.data.getWebMapItem().itemData ? app.data.getWebMapItem().itemData.operationalLayers : [];
				...
				for (var i = app.map.graphicsLayerIds.length - 1; i >= 0; i--) {
					...
					if ((layer.visible == true || layer.visible == undefined) && layer.type == "Feature Layer" && layer.geometryType == "esriGeometryPoint" && !layerName.match(/^mapNotes_/)) {
		...
		function tourPointLayerLoaded(result) {
			...
			_this.setMapExtent(Helper.getWebMapExtentFromItem(app.data.getWebMapItem().item)).then(function() {
				...
				var tourLayer = new esri.layers.GraphicsLayer({
					id: 'mapTourGraphics'
				});
		...
		function selectedPointChange_after(forcedRecord) {
			...
			/**
			 * Customization to add set the TimeSlider to a given interval (as defined in maptour-config.js)
			 * This example uses TimeSlider.setThumbIndexes() to set the value of the slider
			 *
			 */
			...
			// Get value of current Tour Point from config
			val = APPCFG.TIME_VALUES[attributes.getID()];

			if (timeSlider.thumbCount == 1) {
				// Only one index should be set
				if (!$.isArray(val)) {
					// Handle a single value
					indexes = [val];
				} else {
					// Handle a range (only possible if TimeSlider.ThumbCount = 2)
					console.error('Time range is not supported in current configuration');
				}
			} else {
				// Two indexes should be set
				if (!$.isArray(val)) {
					// Handle a single value
					indexes = [val - 1, val + 1];
				} else {
			...
			// Set TimeSlider ThumbIndexes
			timeSlider.setThumbIndexes(indexes);
		...
		this.centerMap = function(geom, zoomLevel) {
			...
			if (!isDesktopModernUI && zoomLevel)
				...
				if (!zoomLevel)
					app.map.centerAt(
						geom.offset(
						offsetX * app.map.__LOD.resolution, -offsetY * app.map.__LOD.resolution));
				else
					app.map.centerAndZoom(
						geom.offset(
						offsetX * app.map._params.lods[zoomLevel].resolution, -offsetY * app.map._params.lods[zoomLevel].resolution),
						zoomLevel);
			}
		}
		...
		function addMapTourBusinessToEsriGraphic() {
			...
			esri.Graphic.prototype.hasBeenUpdated = function() {
				...
				return this.attributes.hasBeenUpdated() || this.geometry.x != originalGeom.x || this.geometry.y != originalGeom.y;
			}
